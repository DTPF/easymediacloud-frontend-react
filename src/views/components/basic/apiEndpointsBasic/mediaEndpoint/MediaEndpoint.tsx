...
function MediaEndpointCollapse({
  endpoint,
  method,
  header,
  token,
        ...
        <div className="example-endpoint__media--description__info">
          ...
          {header === true && (
            <p className="example-endpoint__media--description__info--header">
              <span className="title">Header:</span> Authorization {'{'}
...
function RequestCodeBlock({ requestCodeBlock }: { requestCodeBlock: string }) {
  return (
    <Highlight theme={themes.dracula} code={requestCodeBlock} language="tsx">
...
function ResponseCodeBlock({ responseCodeBlock }: { responseCodeBlock: string }) {
  return (
    <Highlight theme={themes.dracula} code={responseCodeBlock} language="tsx">
      {({ className, style, tokens, getLineProps, getTokenProps }) => (
        <pre style={style}>
          <div style={{ padding: 10 }}>
            {tokens.map((line, i) => (
